...
router.post('/add/:id',function(req,res,next){
    ...
    var sql ='UPDATE auction set Cur_price=? , Buyer_id=? WHERE Auction_id=?;';
     if(req.session.loginId != undefined) {
         conn.query(curbid_sql, [id], function (err, topics) {
             if(topics[0].Cur_price < req.body.current_bid && topics[0].Min_price <= req.body.current_bid  && topics[0].Max_price >= req.body.current_bid && req.session.loginId != topics[0].Seller_id ) {
...
router.post('/', function(req,res,next){
    ...
    //title은 검색 할 제목, category 는 선택한 과목 카테고리
    var title = req.body.search;
    var category = req.body.city2;


    console.log("Title :"+title + category);
    //선택한 카테고리가 all categories 라면 모든 카테고리에서 해당 제목에 대해 검색한다
    if(category=='0')
    {
        var sql = 'SELECT * FROM auction where Title like "%' + title +'%" and Status="입찰대기중"';
        console.log("111" +sql);
    }
    //선택한 카테고리가 all categories가 아니라면 선택한 카테고리에서 해당 제목에 대해 검색한다.
    else {
        var sql = 'SELECT * FROM auction where title like "%' + title + '%" and category="'+ category+'" and Status="입찰대기중"';
        console.log("222" + sql);
    }
